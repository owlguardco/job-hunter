...
const stages = [
  {
    ...
    steps: [
      ...
      { cmd: 'npm run ghost-check',   desc: 'Detect ghost jobs before wasting time applying — scored on 8 signals' },
      { cmd: 'npm run scorecard',     desc: 'Build your evaluation criteria before the offer high clouds your judgment' },
      { cmd: 'npm run momentum',      desc: 'Weekly check-in — real metrics, wins, burnout check, next 3 actions' },,
      { cmd: 'npm run ghost-check',   desc: 'Detect ghost jobs before wasting time applying — scored on 8 signals' },
      { cmd: 'npm run scorecard',     desc: 'Build your personal criteria for evaluating companies — before the offer high' },
      { cmd: 'npm run momentum',      desc: 'Weekly check-in — real metrics, wins, burnout check, next 3 actions' },
  ...
  },
  {
    ...
    steps: [
      ...
      { cmd: 'npm run skill-up',      desc: 'Find which JD requirements are real gates vs. wishlists — with fast close plans' },
      { cmd: 'npm run assessment',    desc: 'Prep for HireVue, take-homes, case studies, technical screens' },
  ...
  },
  {
    ...
    steps: [
      ...
      { cmd: 'npm run panel-decode', desc: 'Decode what each interviewer actually wants — play each room differently' },
      { cmd: 'npm run panel-decode', desc: 'Decode what each interviewer actually wants — and how to play each room' },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { cmd: 'npm run linkedin-post',  desc: '5 posts that build credibility — counterintuitive take, story, insight, industry take, failure' },
      { cmd: 'npm run linkedin-scan',  desc: 'Scan LinkedIn for posts worth commenting on — drafts comments, you approve before anything posts' },
    ],
